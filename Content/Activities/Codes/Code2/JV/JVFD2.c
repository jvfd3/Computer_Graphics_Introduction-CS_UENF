...
typedef struct stUmObjeto {
    vetPontos *Vertices; // vetor de vertices (posicao)
    vetFace *Faces;      // vetor de faces
...
float gKd = 0.2;      // mola: constante de damp
float gKs = 0.25;     // mola constante de spring
float gMassa = 0.001; // masa da particula;
f4d gGrav = {0.0, -9.8, 0.0};
// acelracao de gravidade (0, -9.8, 0) dado em inicial
float gDt = 0.005; // passo de tempo
...
void DesenhaObjeto(umObjeto *obj) {
    ...
    glColor3f(0.0f, 1.0f, 1.0f); // COLORE
    for (i = 0; i < obj->Faces->n; i++) {
        glBegin(GL_POLYGON);
        if (RendMode > 1) {
            calculaNormalFace(&(obj->Faces->face[i]), obj->Vertices);
            // computa normal da face
...
void verficaColisao(umObjeto *obj) {
    vetVetexFace *faceColis = &(objFixo->Faces->face[1]);
    // vetor de indice dos vertices da face de colisao
    f4d *vPosVert = obj->Vertices->vPos;
    statePart *vPart = obj->Parts->mvf;
    f4d vPosPart, vVelN, vVelT; // vetor de posicao da part. em relacao a um
                                // vértice da face de colisao
    int i;
    float s;
    for (i = 0; i < obj->Parts->n; i++) {
        // vPosPart = posParticula - pontoFaceColisao
        vPosPart[0] =
            vPosVert[i][0] - objFixo->Vertices->vPos[faceColis->vInd[1]][0];
        vPosPart[1] =
            vPosVert[i][1] - objFixo->Vertices->vPos[faceColis->vInd[1]][1];
        vPosPart[2] =
            vPosVert[i][2] - objFixo->Vertices->vPos[faceColis->vInd[1]][2];
        ...
        if (s > 0.0) { //  os dois vetor em angulo obtuso
            ...
            // << mudança da velocidade da particula: normal em sentido
            // contrario decompor a velocidade em normal (velN) e tangencial
            // (velT) em relacao a face moduo da vel norma e: vn = vVel .
            // vNormal
...
void computaForcas(umObjeto *obj) {
    ...
    */
    ...
    for (i = 0; i < nMol; i++) {
        ...
        s += (vMol[i].Ks *
              (ll - vMol[i].r)); //  Ks ( || vL || - r ) + Kd (vI . vL) / ||vL||
...
static void passoEDOresolve(umObjeto *obj) {
    ...
    // limpar forca acumuladora de cada particula
...
void geraArestas(umObjeto *obj) {
    // gerando uma lista de molas.. saber quantos e quais sao
    vetVetexFace *ff;
    stLst *first = NULL;
    stLst *node;
    int *vIndVert;
    ...
    while (node) {
        ...
        obj->Molas->mola[i].r = calDistancia(obj->Vertices->vPos[node->a],
                                             obj->Vertices->vPos[node->b]);
    ...
    for (i = 0; i < obj->Molas->n; i++) {
        printf("\n (%d, %d), %4.2f, %4.2f, %4.2f ", obj->Molas->mola[i].a,
               obj->Molas->mola[i].b, obj->Molas->mola[i].r,
               obj->Molas->mola[i].Kd, obj->Molas->mola[i].Ks);
    }
...
void geraParticula(umObjeto *obj) {
    ...
    for (i = 0; i < nn; i++) {
        ...
        obj->Parts->mvf[i].vel[0] = 0.0;
        obj->Parts->mvf[i].vel[1] = 0.0;
        obj->Parts->mvf[i].vel[2] = 0.0;
        obj->Parts->mvf[i].forc[0] = 0.0;
        obj->Parts->mvf[i].forc[1] = 0.0;
        obj->Parts->mvf[i].forc[2] = 0.0;
    }
}
...
int procSolido(char *arch) {
    ...
    if (objeto) {
        objeto = liberaObjeto(objeto);
    }

    if ((objeto = criaObjeto()) == NULL) {
        printf("\n Erro na alocacao de memoria para um objeto");
        return 0;
    }
...
int defineObjetoFixo(char *arch) {
    ...
    if (!objFixo) {
        ...
        for (j = 0; j < objFixo->Faces->n; j++) {
            calculaNormalFace(&(objFixo->Faces->face[j]), objFixo->Vertices);
            // computa normal da face
        }
        ...
        for (j = 0; j < objFixo->Faces->n; j++) {
            ...
            printf(" normal: (%4.2f, %4.2f, %4.2f) \n",
                   objFixo->Faces->face[j].vNormal[0],
                   objFixo->Faces->face[j].vNormal[1],
                   objFixo->Faces->face[j].vNormal[2]);
...
static void Init(void) {
    ...
    // This function is called only once before any other functions are
    // called
    alpha = -20.0;
    beta = 20.0;
    RendMode = 1;
    gGrav[0] = 0.0;
    gGrav[1] = -12.8;
    gGrav[2] = 0.0; // aceleracao de gravidade
    gAnima = 0;
    ...
    glEnable(GL_LIGHT1);
    glEnable(GL_COLOR_MATERIAL); // COLORE

    // enabling both side illumination for the polygons and hidden surface/line
    // removal
...
void MakeShapes(void) {
    /*
        ...
        If the shaopes change in time, the time parameter controllling these
        changes can be defined and changed in function draw()

        Since the axes are to be drawn as solid lines, we disable lighting for
        them.
    */
    ...
    /*
       The polygons are rendered differently depending on the selected
       rendering modes: which are wireframe, flat shading, and smooth shading.
       Notice how the edges visually disappear when smooth rendering is
       selected. Default material settings are used. You may define your own
       material properties.
    */
    if (RendMode == 1) {
...
static void idle(void) {
    // This function will call draw() as frequent as possible thus enabling us
    // to make interaction and animation.
    draw();
}
...
void processMenuEvents(int option) {
    gAnima = 0;
    char dataRupee[] =
        "B:/ICG/Content/Activities/Codes/Code2/JV/data/dadosJVFD.txt";
    char dataCube[] =
        "B:/ICG/Content/Activities/Codes/Code2/Base/data/dadosCubo.txt";
    char dataCubeBase[] =
        "B:/ICG/Content/Activities/Codes/Code2/Base/data/dadosCuboBase.txt";
    char dataPyramid[] =
        "B:/ICG/Content/Activities/Codes/Code2/Base/data/dadosPiramide.txt";
    if (option == 1) {
        procSolido(dataCube);
    } else {
        if (option == 2) {
            procSolido(dataPyramid);
        } else {
            if (option == 3) {
                gAnima = 1;
            } else {
                if (option == 4) {
                    procSolido(dataRupee);
                }
            }
        }
    }
    defineObjetoFixo(dataCubeBase);
    glutPostRedisplay();
}
...
void createGLUTMenus() {
    ...
    glutAddMenuEntry("Solido: Rupee", 4);
