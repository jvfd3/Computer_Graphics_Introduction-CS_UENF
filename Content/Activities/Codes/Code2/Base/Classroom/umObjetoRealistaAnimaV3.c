...
void verficaColissao(umObjeto *obj) {
	...
	f4d vPosPart, vVelN, vVelT;	// vetor de posicao da part. em relacao a um vértice da face de colisao
	int i;
	float s;
	for(i=0; i<obj->Parts->n; i++) {
		...
		if (s > 0.0)  {  //  os dois vetor em angulo obtuso
			...
			// << mudança da velocidade da particula: normal em sentido contrario 
			// decompor a velocidade em normal (velN) e tangencial (velT) em relacao a face
			    // moduo da vel norma é: vn = vVel . vNormal
...
static void passoEDOresolve(umObjeto *obj) {
	...
	// fimpar força acumuladora de cada particula
...
{
	// gerando uma lista de molas.. saber quantos e quais são
	vetVetexFace *ff;
	stLst *first = NULL;
	stLst *node;
	int *vIndVert;
