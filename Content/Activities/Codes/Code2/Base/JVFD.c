...
void DesenhaObjeto(umObjeto *obj) {
    ...
    for (i = 0; i < obj->Faces->n; i++) {
        glBegin(GL_POLYGON);
        if (RendMode > 1) {
            calculaNormalFace(&(obj->Faces->face[i]),
                              obj->Vertices); // computa normal da face
...
static void Init(void) {
    ...
    beta = 20.0;
    RendMode = 1;
    //    gGrav[0] = 0.0;    gGrav[1] = -9.8; gGrav[2] = 0.0;  // acelercao de
    //    gravidade
    gAnima = 0;
...
void processMenuEvents(int option) {
    gAnima = 0;
    if (option == 1) {
        procSolido("data/dadosCubo.txt"); // data/dadosCubo.txt
    } else {
        if (option == 2) {
            procSolido("data/dadosPiramide.txt");
        } else {
            if (option == 3) {
                gAnima = 1;
            }
    ...
    defineObjetoFixo("data/dadosCuboBase.txt");
...
void createGLUTMenus() {
    ...
    glutAddMenuEntry("Soldio Cubo", 1);
